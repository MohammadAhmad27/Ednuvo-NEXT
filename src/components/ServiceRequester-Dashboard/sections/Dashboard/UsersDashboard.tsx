...
import JobPostDialog from "@/components/ui/Dialogs/JobPostDialog";
...
const UsersDashboard = () => {
  ...
  return (
    <>
      <div className="w-full h-full flex flex-col gap-4 pt-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <p className="text-[18px] font-normal text-[#2D2D2D]">
            Welcome, <span className="font-semibold">Hassan Al-Omari</span>{" "}
            Here’s your business overview.
          </p>
          <button
            onClick={() => setIsModalOpen(true)}
            className="bg-primary text-[14px] font-medium text-white text-center rounded-full px-8 py-[6px]"
          >
            Post a Job
          </button>
        </div>
      ...
      <JobPostDialog open={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </>
  );
};
